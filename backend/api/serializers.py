...
class RecipeCRUDSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        for ingredient_data in ingredients:
            ingredient = ingredient_data['ingredient']
            amount = ingredient_data['amount']
            instance.recipe_ingredients.create(ingredient=ingredient, amount=amount)
        return instance
...
#         tags_data = validated_data.pop('tags', [])
#         recipe = Recipe.objects.create(**validated_data)
#         recipe.tags.set([tag['id'] if isinstance(tag, dict) else tag for tag in tags_data])
#         for ingredient_data in ingredients:
...
#         instance.cooking_time = validated_data.get('cooking_time', instance.cooking_time)
#         instance.save()
#         instance.tags.set([tag['id'] if isinstance(tag, dict) else tag for tag in tags_data])
#         instance.recipe_ingredients.all().delete()
#         for ingredient_data in ingredients:
#             ingredient = ingredient_data.get('ingredient')
#             amount = ingredient_data.get('amount')
...
# class UserSerializer(serializers.ModelSerializer):
#     is_subscribed = serializers.SerializerMethodField()
#     avatar = serializers.ImageField(read_only=True)  # При условии, что поле avatar есть
...
#             # Логика подписки пользователя (например, возвращает True/False)
